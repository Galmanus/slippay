...
(`slippay-subscription.charge` → nested `token.transfer`). That flow is
...
Slippay backend code. 13 unit tests cover them deterministically.
...
the merchant-pull flow against the production slippay-subscription

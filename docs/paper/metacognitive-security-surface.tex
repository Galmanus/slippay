...
vulnerabilities is provably incomplete (Theorem~1, under three explicitly
...
We make three assumptions, each empirically grounded in the architecture
of Sections~3.2--3.3. Let \(\Omega_M\) denote the space of metacognitive
states (the range of \(M(\cdot,\theta)\)).
...
\(S \subseteq \Omega_M\) on which the learned ``a constraint applies''
representation is reliably active, with activation
\(\sigma : \Omega_M \to [0,1]\) satisfying \(\sigma(m)\to 1\) on \(S\) and
\(\sigma(m) \le \eta\) for \(m \notin S\), where \(\eta\) is the small
out-of-support generalization residual; and \(S \neq \Omega_M\) for any
finite training run.

\textbf{Assumption A2 (Latent capability; suppression, not erasure).}
Alignment suppresses harmful generation without removing the underlying
capability: there exist a metacognitive state \(m_h \in \Omega_M\) and a
harmful target \(y_h\) with \(P(y_h \mid x, M\!=\!m_h) \ge p\) for some
\(p>0\). Equivalently, safety relies on keeping \(M\) out of \(m_h\)
(the representation of A1), not on incapacity.

\textbf{Assumption A3 (Metacognitive reachability).} Under multi-turn
adversarial context the induced map \(x \mapsto M(x,\theta)\) is
controllable into a neighborhood of \(m_h\) lying outside \(S\): there
exists \(x^* \in \mathcal{A}_M\) with
\(m^* := M(x^*,\theta) \in B_\delta(m_h)\setminus S\). This is the
operational content of the context-sensitivity of Section~3.3.

\textbf{Theorem 1 (Incompleteness of generative-layer alignment).} Under
A1--A3, for any aligned model \(\mathcal{M}_{\text{aligned}}\) there
exists an adversarial input \(x^* \in \mathcal{A}_M\) such that
\[ P_{\text{aligned}}(y_{\text{harmful}} \mid x^*) \;\ge\; \varepsilon \;>\; 0, \]
and \(\varepsilon\) does not vanish as generative-layer alignment
strength increases.

\emph{Proof.} By A3 pick \(x^*\) with
\(m^* = M(x^*,\theta) \in B_\delta(m_h)\setminus S\). The aligned output
probability factors through the metacognitive state into a suppressed and
an unsuppressed branch:
\[ P_{\text{aligned}}(y_h \mid x^*) = \sigma(m^*)\,\underbrace{P_{\text{supp}}(y_h\mid x^*)}_{\approx\,0} + \big(1-\sigma(m^*)\big)\,P(y_h\mid x^*,m^*). \]
Since \(m^* \notin S\), A1 gives \(\sigma(m^*)\le \eta\). Assuming
\(P(y_h\mid\cdot)\) is \(L\)-Lipschitz in the metacognitive state on the
(open) harmful basin around \(m_h\), A2 gives
\(P(y_h\mid x^*,m^*) \ge p - L\delta\). Therefore
\[ P_{\text{aligned}}(y_h\mid x^*) \;\ge\; (1-\eta)\,(p - L\delta) \;=:\; \varepsilon \;>\;0 \]
for any \(\delta < p/L\) and \(\eta<1\). Finally, increasing
generative-layer alignment strength enlarges \(S\) and lowers \(\eta\)
\emph{on \(S\)}, but by A1 cannot achieve \(S=\Omega_M\) with a finite
signal; the state \(m^*\) outside \(S\) therefore persists and
\(\varepsilon\) is bounded away from \(0\) independently of alignment
strength. \(\qquad\blacksquare\)

\textbf{Remark (the assumptions are mild).} A1 is the finiteness of any
real training run; A2 is the widely observed capability--alignment gap (a
useful model cannot be made to \emph{not know} \(y_h\)); A3 is precisely
the empirical content of Sections~5--6, where POMDP-adaptive and
progressive-context attacks reach such \(m^*\) at 76--89\% success. The
theorem thus converts an empirical regularity into a structural
necessity: the gap is not a coverage bug to be patched but a consequence
of supervising \(G\) while leaving \(M\) unconstrained.

\emph{Falsification condition:} This theorem fails if a training method
can be shown to make \(M\) itself invariant to adversarial manipulation.
Specifically, if representation engineering or similar methods can
produce models where the metacognitive representation of ``constraint
applies'' is robust to all context manipulations, Theorem 1 fails for
that class of attacks. We note no such method currently exists, but its
existence would be a meaningful empirical discovery.
...
\item
  ...
  belief distributions to actions
\end{itemize}

The attacker's problem is to find
\(\pi^* = \arg\max_\pi \mathbb{E}[\sum_t R(s_t, a_t)]\) --- to maximize
the cumulative reward (i.e., the probability of eliciting harmful output
over time).
...
\begin{quote}
\textbf{Concrete Example --- Class E POMDP belief update dynamics (Metis
framework, arXiv:2605.10067):} The Metis framework's empirical results
reveal a characteristic curve that validates the POMDP model. Success
rates are not static; they improve across turns in a pattern consistent
with Bayesian belief convergence:
...
The Metis framework also reports that the adaptive attacker
substantially outperforms the best static attack baseline against the
same models: +31 percentage points average across frontier models. This
gap is the empirical measure of the metacognitive attack surface's size
--- it represents the proportion of attacks that succeed only because of
adaptive metacognitive targeting.
\end{quote}

\begin{quote}
\textbf{Concrete Example --- POMDP vs.~static attack efficiency
comparison (Metis, 2605.10067):} Against o1 (OpenAI's extended reasoning
model, explicitly designed for aligned reasoning): static
best-performing attack achieves 45\% success; Metis POMDP achieves 76\%
success. The 31-point gap implies that approximately 1 in 3 of o1's
successful defenses against static attacks are broken by adaptive
metacognitive targeting. This is a direct measure of metacognitive
vulnerability that is invisible to static evaluation.
\end{quote}
...
\begin{quote}
\textbf{Concrete Example --- Metis model-by-model breakdown
(arXiv:2605.10067, Table 2):} The paper reports per-model success rates
that reveal important heterogeneity. Models with the highest success
rates against Metis are NOT the most safety-trained ones by conventional
metrics --- they are the ones with the most rigid (least
contextually-sensitive) metacognitive state spaces. This is a
counterintuitive finding: the model property that protects against
metacognitive attacks (state rigidity) is anti-correlated with the
property most valued in helpfulness evaluations (contextual
sensitivity). The practical implication: optimizing for helpfulness and
contextual sensitivity may structurally increase metacognitive attack
surface. This tradeoff is unaddressed in all current alignment
frameworks.
\end{quote}
...
Perez \& Ribeiro (2022) established the baseline for multi-turn attack
effectiveness. Their ``Many-shot jailbreaking'' paper demonstrated that
providing many examples of the model complying with progressively more
harmful requests within a single context window significantly increases
the probability of the model complying with a final harmful request.
...
\begin{quote}
\textbf{Concrete Example --- Many-shot jailbreaking dose-response curve
(Anthropic, ``Many-Shot Jailbreaking,'' 2024):} The paper reports that
the probability of eliciting a harmful response follows a dose-response
curve as a function of the number of in-context examples of prior
compliance: 1 example: \textasciitilde5\% above baseline; 10 examples:
\textasciitilde20\% above baseline; 50 examples: \textasciitilde45\%
above baseline; 100+ examples: \textasciitilde65-70\% above baseline.
The curve is roughly logarithmic --- early examples have high marginal
...
\textbf{Expected efficacy:} 60-75\% reduction in vulnerability to known
...
\textbf{Expected efficacy:} High (70-85\%) for attacks targeting the
...
\textbf{Expected efficacy:} 65-80\% reduction in successful attacks,
...
\textbf{Expected efficacy:} Particularly effective against Class F
...
\textbf{Expected efficacy:} 55-70\% reduction in Class D and E attack
...
a preprint and has not been peer reviewed. All empirical claims about
attack success rates are sourced from cited literature. Falsifiable
predictions in Section 9.3 are novel contributions of this work.}

...
that regenerates the same obligations without a solver. We are deliberate
about scope. AXL today is an \emph{offline verifier}, not a deployed control:
the on-chain contract enforces the same $2\times$ bound by hard-coded logic
and treats the policy hash as provenance only; no continuous-integration gate
yet couples proof to deployment. We state these gaps as falsifiable
conditions, because the contribution is the verifiable artifact and its
construction, not a claim that it is currently load-bearing in production.
\par\vspace{0.6em}
...
faithful browser re-verifier. The contribution is \emph{not} a deployed control:
\S\ref{sec:scope} states plainly that the on-chain contract enforces the bound
by hard-coded arithmetic and consumes no certificate, that no CI gate yet
couples proof to deployment, and that the underlying SMT model is small enough
($\approx$40 lines) that the proof itself is not a barrier to entry. We treat
these as falsifiable gaps to close, not as features to obscure.
...
suspicious?'') are out of scope and still require a model in the loop. Beyond
that boundary, we name the gaps between the artifact and a deployed control,
each as a falsifiable statement.

\begin{itemize}
\item \textbf{No CI gate couples proof to deployment.} There is no workflow
  that runs \texttt{verify-cert} on change. \emph{Falsifiable:} until a
  required check rejects a merge whose policy and certificate disagree, the
  proof can silently drift from the deployed contract on any commit.
\item \textbf{The on-chain constant is hard-coded, not certificate-driven.}
  The contract enforces $2\times$ as a literal. \emph{Falsifiable:} if AXL
  proved $K=3$ for a revised policy, the contract would not track it without a
  manual edit; proof and enforcement are not yet a single source of truth.
\item \textbf{The policy hash is provenance, not a gate.} \texttt{ssl\_hash}
  is pinned immutably but never interpreted on-chain. \emph{Falsifiable:} no
  spend is rejected on the basis of a hash mismatch with a certified policy.
\item \textbf{The proof is not a moat.} The SMT model is $\approx$40 lines; a
  competent formal-methods engineer reproduces it in engineer-weeks. The only
  durable advantage would be \emph{adoption} of the certificate as a
  required-to-participate artifact (a second prover, an insurer, an on-chain
...
\item \textbf{The solver is a runtime dependency.} \texttt{prove},
  ...
  package). This is intentional and fail-closed --- absence refuses --- but it
  means a consumer without a solver can check only the binding and the
  obligation text, not the discharge.
\end{itemize}
...
deterministic re-verification, drift detection --- is real and tested. The
forcing function that would make it load-bearing in production is the work that
remains.
...
bound to its policy by a content hash so that drift is detectable. We have been
explicit that the scheme is, today, an offline verifier rather than a deployed
control: the on-chain contract enforces the same bound by hard-coded
arithmetic and consumes no certificate, and no CI gate yet couples proof to
deployment. Those are the falsifiable conditions whose resolution would turn a
verifiable artifact into a load-bearing one. The thesis is narrow and, we
think, correct: for money moving between agents that do not trust each other, a
bound the counterparty can re-check beats a bound it must take on faith.

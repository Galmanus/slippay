# Deploy — how SlipPay is actually deployed
...
Both `app.slippay.cc` and `api.slippay.cc` run on a single VPS, behind nginx,
...
- One VPS.
- nginx terminates TLS and routes `app.slippay.cc` and `api.slippay.cc`.

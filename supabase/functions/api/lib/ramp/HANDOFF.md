...
Edit `/opt/slippay-backend/.env` and add:
...
```
cd /opt/slippay-backend && pm2 reload ecosystem.config.cjs --only slippay-api && pm2 save
```

Verify: `curl -s https://api.slippay.cc/api/v1/offramp/status` should now show

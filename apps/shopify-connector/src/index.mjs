// SlipPay × Shopify — Payments App connector.
//
// Maps Shopify's Payments Apps payment-session flow onto SlipPay's API.
// Third platform adapter (after WooCommerce + VTEX): same motor
// (POST /api/v1/orders → checkout_url), different platform contract.
//
// Shopify flow: buyer picks "SlipPay" at checkout → Shopify POSTs a payment
// session to this app → we create a SlipPay order and return { redirect_url =
// checkout_url } → buyer pays → SlipPay webhook → we call Shopify
// paymentSessionResolve to finalize.
//
// Scope: WORKING connector proving the mapping against the live SlipPay
...
const sessions = new Map(); // shopifySessionId → { slippayOrderId, status }
...
async function createSlippayOrder({ apiKey, amount, reference }) {
...
// Finalize the Shopify payment session once SlipPay confirms payment.
...
const server = http.createServer(async (req, res) => {
  ...
  // Payment session: buyer chose SlipPay → create order, redirect to checkout.
  if (req.method === "POST" && path === "/payment_sessions") {
    ...
    try {
      const { order, checkout_url } = await createSlippayOrder({ apiKey, amount, reference: String(sessionId) });
      sessions.set(sessionId, { slippayOrderId: order.id, status: "pending", checkoutUrl: checkout_url });
      // Offsite redirect: Shopify sends the buyer to redirect_url.
      return json(res, 201, {
        redirect_url: checkout_url,
        slippay_order: order.id,
        message: `SlipPay order ${order.id} (${order.brl_amount} BRL → ${order.usdc_amount} USDC)`,
      });
    } catch (e) {
  ...
  // SlipPay webhook → resolve the Shopify session (finalize as paid).
  if (req.method === "POST" && path === "/slippay-webhook") {
    ...
    const entry = [...sessions.entries()].find(([, s]) => s.slippayOrderId === orderId);
    if (entry && paid) {
  ...
  if (req.method === "GET" && path === "/health") return json(res, 200, { ok: true, slippay: SLIPPAY_API_BASE, shopResolve: Boolean(SHOP && SHOP_TOKEN) });

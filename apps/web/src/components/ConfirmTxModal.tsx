    ...
    >
      <div className="bg-[#f1eee7] text-[#0a0a0a] w-full max-w-md mx-4 p-8 shadow-2xl">
        ...
        <div className="space-y-6">
          {payments.length === 0 && (
            <div className="text-sm text-[#0a0a0a]/70">
              No payment operations detected in this transaction.
            </div>
          )}
          {payments.map((op, i) => (
        ...
        <div className="mt-6 text-[10px] uppercase tracking-[0.14em] text-[#0a0a0a]/50 border border-[#0a0a0a]/15 px-3 py-2">
          Verify all details before confirming. This action cannot be undone.
        </div>
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        <div className="font-mono text-sm break-all">{op.destination ?? "—"}</div>
      </div>
      <div className="flex gap-8">
        <div>
          <Label>Amount</Label>
          <div className="text-2xl font-medium tabular-nums">{op.amount ?? "—"}</div>
        </div>
        <div>
          <Label>Asset</Label>

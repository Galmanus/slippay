    ...
    <div className="min-h-screen bg-[#0E0D0B] text-[#F1EEE7] grain overflow-x-hidden">
      ...
      <header className="px-6 md:px-12 py-7 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#F1EEE7]/45">
            <button onClick={() => setLang("pt")} className={lang === "pt" ? "text-[#F1EEE7] font-medium" : "hover:opacity-80"}>PT</button>
            <span className="opacity-30 mx-1">/</span>
            <button onClick={() => setLang("en")} className={lang === "en" ? "text-[#F1EEE7] font-medium" : "hover:opacity-80"}>EN</button>
          </div>
      ...
      <main className="max-w-[680px] mx-auto px-6 md:px-12 pt-12 md:pt-20 pb-28">
        <div className="flex items-baseline gap-3 font-mono text-[11px] uppercase tracking-[0.3em] text-[#F1EEE7]/40">
          <span className="text-[#F1EEE7]/55">001</span><span className="h-px w-8 bg-current opacity-40" /><span>{t.stamp}</span>
        </div>
        <h1 className="mt-10 font-bold uppercase tracking-[-0.05em] leading-[0.85] text-[clamp(2.5rem,9vw,5.5rem)]" style={display}>
          {t.h1a}<br /><span className="text-[#F1EEE7]/40">{t.h1acc}</span>
        </h1>
        <p className="mt-8 text-lg md:text-xl leading-relaxed max-w-[44ch] text-[#F1EEE7]/70">{t.sub}</p>

        {/* converter */}
        <div className="mt-12 rounded-3xl border border-[#F1EEE7]/12 bg-[#16140F] p-6 md:p-8">
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#F1EEE7]/45">{t.youSend}</div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl text-[#F1EEE7]/55" style={display}>R$</span>
            <input
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {PRESETS.map((p) => (
              <button key={p} onClick={() => setBrl(p)}
                className={`rounded-full px-4 py-1.5 text-[12px] tabular-nums border transition-colors ${brl === p ? "bg-[#FDDA24] text-[#0a0a0a] border-[#FDDA24]" : "border-[#F1EEE7]/20 hover:border-[#F1EEE7]/50"}`}>
                R$ {p.toLocaleString("pt-BR")}
              </button>
            ))}
          </div>

          <div className="my-6 h-px bg-[#F1EEE7]/10" />

          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#F1EEE7]/45">{t.youGet}</div>
          <div className="mt-2 flex items-baseline gap-2.5">
            <span className="text-3xl text-[#F1EEE7]/45" style={display}>$</span>
            ...
            <span className="text-lg text-[#F1EEE7]/45 self-end mb-1.5">USDC</span>
          </div>
          <div className="mt-3 font-mono text-[11px] uppercase tracking-[0.14em] text-[#F1EEE7]/40">
            {quote ? t.rate(quote.quotedRate) : "…"}
          </div>
          <p className="mt-3 text-[12px] leading-relaxed text-[#F1EEE7]/50 max-w-[48ch]">
        ...
        {acct && transakConfigured() ? (
          ...
          <div className="mt-8 text-[12px] uppercase tracking-[0.2em] text-[#F1EEE7]/40">{t.soon}</div>
        )}

        <p className="mt-6 text-[13px] text-[#F1EEE7]/55 max-w-[46ch] flex items-start gap-2">

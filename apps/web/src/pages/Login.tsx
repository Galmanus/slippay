  ...
  return (
    <main className="max-w-sm mx-auto p-8">
      ...
      <form onSubmit={async (e) => {
        ...
        <button disabled={loading} className="w-full bg-emerald-500 disabled:opacity-50 text-black py-2 rounded font-semibold">{loading ? "..." : "log in"}</button>
        {err && <div className="text-red-400 text-sm">{err}</div>}
      </form>
      <Link to="/signup" className="text-sm text-zinc-400 mt-4 block">no account? sign up</Link>
    </main>
  );
}
